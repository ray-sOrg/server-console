...
EXERCISES = [
    ...
    ('俯卧撑', 'strength', '胸/三头', '自重', 'reps', '身体保持一条直线，胸部主动下沉并完整推起。', '塌腰或肩前侧不适时减少次数。', '全组达到15次后继续增加标准总次数。'),
    ...
    ('实力推', 'skill', '背/肩/手臂', '单杠', 'reps', '从杠下完成拉起、过渡并稳定撑到杠上，全程保持控制。', '不练到力竭；肩肘疼痛或动作无法完整时停止。', '优先增加动作完整的总次数，不追求失败次数。'),
    ...
    ('哑铃负重卷腹', 'strength', '腹', '哑铃', 'reps', '保持骨盆稳定，用腹部卷起躯干，哑铃贴近胸前。', '颈部代偿或腰部不适时减重。', '12–20次稳定后小幅增加重量。'),
...
WEEK = [
    (1, '上肢A', '胸 / 推 / 三头 + 俄挺技能', False, 85, [
        ...
        ('哑铃飞鸟', 3, (10, 15), None, (1, 2), 90, '先加次数', '使用轻中重量，控制胸部拉伸。'),
    ...
    (2, '下肢A', '深蹲 / 后链 / 腹', False, 80, [
        ...
        ('健腹轮', 4, (6, 12), None, (1, 2), 75, '先加次数', '以腰不塌为准。'),
        ('悬垂举腿', 3, (10, 15), None, (1, 2), 75, '先加次数', '骨盆后卷，以不摆动为准。'),
    ]),
    (3, '拉力A', '引体 / 划船 / 二头 + 俄挺技能', False, 85, [
        ('俄挺前倾支撑', 3, None, (10, 20), None, 60, '先加时间', '保持肩胛前伸，逐步增加可控前倾。'),
        ('引体向上', 5, (5, 10), None, (1, 2), 120, '先加总次数', '先达到总计30次，再逐步提高到5组各10次。'),
    ...
    (5, '上肢B', '上胸 / 背 / 手臂 + 实力推技能', False, 85, [
        ('实力推', 5, (1, 3), None, None, 150, '先加总次数', '只记录完整动作，不练到失败。'),
        ('上斜卧推', 4, (6, 10), None, (1, 2), 120, '双进阶', '上胸主项。'),
        ('俯卧撑', 3, (8, 15), None, (1, 2), 90, '先加次数', '保持完整活动范围和身体稳定。'),
    ...
    (6, '下肢B', '腿 / 核心 / 握力', False, 70, [
        ...
        ('悬垂举腿', 4, (10, 15), None, (1, 2), 75, '先加次数', '骨盆后卷，以不摆动为准。'),
        ('哑铃负重卷腹', 3, (12, 20), None, (1, 2), 75, '双进阶', '动作稳定后再小幅加重。'),
